...
/**
 * The <code>Complex</code> class provides functionality for the complex number system where
 * <i>i</i> is not ignored. An object of type
 * <code>Complex</code> contains two fields whose type is <code>double</code>.
 * <p>
 * By necessity, this class provides methods for addition, subtraction, multiplication, division,
 * exponential, and other trigonometric and logarithmic functions.
 */
...
public class Complex {
    ...
    public Complex pow(double power) {
        ...
        return new Complex(Math.pow(r, power) * Math.cos(power * theta),
    ...
    public Complex pow(Complex p) {
        ...
        double newReal =
                Math.pow(ns, c / 2) / dDenom * (Math.cos(dArg) * Math.cos(cArg) * Math.log(ns)
                        - Math.sin(dArg) * Math.sin(cArg) * Math.log(ns));
        double newImag =
                Math.pow(ns, c / 2) / dDenom * (Math.cos(dArg) * Math.sin(cArg) * Math.log(ns)
